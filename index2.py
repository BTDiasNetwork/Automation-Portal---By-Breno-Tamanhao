...
def buscar_prefixos_disponiveis(prefixo_pai, mascara):
    """
    Busca prefixos disponíveis dentro do prefixo pai com a máscara exata.

    O pynetbox ignora o parâmetro prefix_length em .list(), então:
    1. Buscamos todos os sub-prefixos já alocados dentro do pai via API
    2. Calculamos manualmente quais /mascara ainda estão livres
    3. Retornamos apenas os prefixos com a máscara exata solicitada
    """
    try:
        rede_pai = ipaddress.ip_network(prefixo_pai, strict=False)

        # Todos os prefixos já alocados dentro do pai (qualquer máscara)
        ...
        for candidato in candidatos:
            # Um candidato está disponível se nenhum prefixo alocado
            # o sobrepõe (overlap) — nem contém, nem é contido
            sobreposto = any(
...
with st.sidebar:
    ...
    st.write(f"**Usuário:** {usuario_info['usuario']}")
...
st.markdown("<h1 style='text-align: center;'>🌐 Portal de Automação NetBox + AWX</h1>",
            unsafe_allow_html=True)
st.markdown(
...
with st.expander("2️⃣ Criar VRF", expanded=(
...
with st.expander("3️⃣ VLAN Internal", expanded=(
...
with st.expander("4️⃣ VLAN Transit", expanded=(
...
with st.expander("5️⃣ Prefixo /24", expanded=(
    ...
    if not st.session_state.tenant_criado:
        ...
        if st.button("🔎 Buscar /24", key="btn_buscar_24"):
            with st.spinner("Buscando prefixos /24 disponíveis..."):
        ...
        if st.session_state.get('prefixos_24'):
            ...
            if disp:
                st.success(f"✅ {len(disp)} prefixos /24 disponíveis")
                ...
                st.warning("⚠️ Nenhum prefixo /24 disponível")
...
with st.expander("6️⃣ Prefixo /30", expanded=(
    ...
    if not st.session_state.tenant_criado:
        ...
        if st.button("🔎 Buscar /30", key="btn_buscar_30"):
            with st.spinner("Buscando prefixos /30 disponíveis..."):
        ...
        if st.session_state.get('prefixos_30'):
            ...
            if disp:
                st.success(f"✅ {len(disp)} prefixos /30 disponíveis")
                for i, p in enumerate(disp[:20], 1):
                    st.write(f"{i}. {p}")
                if len(disp) > 20:
                    st.caption(f"... e mais {len(disp) - 20}")
                ...
                st.warning("⚠️ Nenhum prefixo /30 disponível")
...
vrf_next_hop: {extra_vars['vrf_next_hop']}""", language="yaml")
    ...
    if err:
        ...
        st.warning("⚠️ Nenhum Job Template disponível")
...
root@portal-web-network:~# ^C
root@portal-web-network:~# cat venv/portal-automation/app/index.py | more
# -*- coding: utf-8 -*-
import streamlit as st
import pynetbox
import requests
    ...
    if st.session_state.tenant_criado:
        ...
        with col2:
            if groups:
                ...
                group_id = None
root@portal-web-network:~# nano venv/portal-automation/app/index.py
root@portal-web-network:~# nano venv/portal-automation/app/index.py
root@portal-web-network:~# nano venv/portal-automation/app/index.py
root@portal-web-network:~# nano venv/portal-automation/app/index.py
root@portal-web-network:~#
root@portal-web-network:~#
root@portal-web-network:~#
root@portal-web-network:~#
root@portal-web-network:~# init 6
root@portal-web-network:~#
login as: root
root@10.100.250.100's password:
 ...
 * Support:        https://ubuntu.com/pro
 ...
 System information as of Thu Feb 26 13:36:59 UTC 2026

  System load:  0.02              Processes:             118
  Usage of /:   9.3% of 37.70GB   Users logged in:       0
  Memory usage: 9%                IPv4 address for ens3: 10.100.250.100
  Swap usage:   0%
...
Last login: Thu Feb 26 12:51:41 2026 from 10.100.250.37
root@portal-web-network:~#
root@portal-web-network:~#
root@portal-web-network:~#
root@portal-web-network:~# cat venv/
bin/               etc/               include/           lib/               lib64/             portal-automation/ pyvenv.cfg         share/
root@portal-web-network:~# cat venv/
bin/               etc/               include/           lib/               lib64/             portal-automation/ pyvenv.cfg         share/
root@portal-web-network:~# cat venv/
bin/               etc/               include/           lib/               lib64/             portal-automation/ pyvenv.cfg         share/
root@portal-web-network:~# cat venv/portal-automation/
%                      GUIA_DEPLOY.txt.txt    ldap_auth-original.py  nano
.streamlit/            app/                   ldap_auth.py           script_portal.sh
root@portal-web-network:~# cat venv/portal-automation/
%                      GUIA_DEPLOY.txt.txt    ldap_auth-original.py  nano
.streamlit/            app/                   ldap_auth.py           script_portal.sh
root@portal-web-network:~# cat venv/portal-automation/
%                      GUIA_DEPLOY.txt.txt    ldap_auth-original.py  nano
.streamlit/            app/                   ldap_auth.py           script_portal.sh
root@portal-web-network:~# cat venv/portal-automation/app/index.py
# -*- coding: utf-8 -*-
import streamlit as st
import pynetbox
import requests
