...
# ============================================================================
# IDENTIFICA\UffffffffO DO USU\UffffffffIO
# ============================================================================
...
<style>
    ...
    /* Bot\Ufffffffflogout vermelho */
    [data-testid="stSidebar"] div[data-testid="stButton"]:last-child button {
        background-color: #dc2626;
        color: white;
        border: none;
    }
    [data-testid="stSidebar"] div[data-testid="stButton"]:last-child button:hover {
...
def buscar_prefixos_disponiveis(prefixo_pai, mascara):
    """
    Busca prefixos dispon\Uffffffffis dentro do prefixo pai com a m\Uffffffffara exata.

    O pynetbox ignora o par\Ufffffffftro prefix_length em .list(), ent\Uffffffff
    1. Buscamos todos os sub-prefixos j\Ufffffffflocados dentro do pai via API
    2. Calculamos manualmente quais /mascara ainda est\Ufffffffflivres
    3. Retornamos apenas os prefixos com a m\Uffffffffara exata solicitada
    """
    try:
        rede_pai = ipaddress.ip_network(prefixo_pai, strict=False)

        # Todos os prefixos j\Ufffffffflocados dentro do pai (qualquer m\Uffffffffara)
        ...
        # Gera todos os candidatos /mascara dentro do pai
        candidatos = list(rede_pai.subnets(new_prefix=mascara))

        disponiveis = []
        for candidato in candidatos:
            # Um candidato est\Uffffffffispon\Uffffffffl se nenhum prefixo alocado
            # o sobrep\Uffffffffoverlap) \Uffffffff nem cont\Uffffffff nem \Uffffffffontido
            sobreposto = any(
                candidato.overlaps(alocado)
...
with st.sidebar:
    st.markdown("### ?? Sess\UffffffffAtiva")
    st.write(f"**Nome:** {usuario_info['nome']}")
    st.write(f"**Usu\Uffffffffo:** {usuario_info['usuario']}")
    if usuario_info['email']:
        st.write(f"**E-mail:** {usuario_info['email']}")
    st.markdown("---")
    st.caption("?? Autenticado via LDAP / Nginx")
    st.markdown("---")
    ...
    if st.button("?? Sair", use_container_width=True, key="btn_logout"):
        ...
        (function() {
            ...
            xhr.open('GET', '/_logout', false, 'logout', 'logout');
...
st.markdown("<h1 style='text-align: center;'> Portal de Automa\Uffffffff</h1>",
            unsafe_allow_html=True)
st.markdown(
    f"<p style='text-align: center; font-size: 18px; color: #64748b;'>"
    f"Bem-vindo, {usuario_info['nome']}!</p>",
    unsafe_allow_html=True
)
st.markdown("---")
    ...
    try:
        ...
        if templates_check:
            ...
            <div style='background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);
                        padding: 20px; border-radius: 10px; color: white;'>
                <h3 style='margin: 0; color: white;'>?? AWX</h3>
                <p style='margin: 5px 0; font-size: 14px;'>{len(templates_check)} Job Templates</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.warning("?? AWX n\Uffffffffconfigurado")
    except Exception:
        st.warning("?? AWX n\Uffffffffconfigurado")
...
for key in ['tenant_criado', 'vrf_criada', 'vlan_internal_alocada',
            'vlan_transit_alocada', 'prefixo_24_alocado', 'prefixo_30_alocado']:
...
st.write(f"**Progresso:** {etapas_completas}/6 etapas conclu\Uffffffffs")
st.markdown("---")
...
with st.expander("1?? Criar Tenant", expanded=(st.session_state.tenant_criado is None)):
    if st.session_state.tenant_criado:
        ...
        with col2:
            if groups:
                ...
                group_id = None
            desc = st.text_area("Descri\Uffffffff:", key="desc_tenant")
...
with st.expander("2?? Criar VRF", expanded=(
        st.session_state.tenant_criado is not None and st.session_state.vrf_criada is None)):
    if not st.session_state.tenant_criado:
        st.warning("?? Crie um tenant primeiro")
        ...
        with col2:
            desc_vrf = st.text_area("Descri\Uffffffff:", key="desc_vrf")
...
with st.expander("3?? VLAN Internal", expanded=(
        st.session_state.vrf_criada is not None and st.session_state.vlan_internal_alocada is None)):
    if not st.session_state.tenant_criado:
        st.warning("?? Crie um tenant primeiro")
        ...
        with col1:
            inicio = st.number_input("In\Uffffffffo:", 1, 4094, 100, key="vlan_int_inicio")
        with col2:
        ...
        if st.session_state.get('vlans_int_disp'):
            ...
            st.success(f"? {len(disp)} VLANs dispon\Uffffffffis")
            st.write(', '.join(map(str, disp[:30])))
            vlan_sel  = st.selectbox("VLAN:", disp[:50], key="vlan_int_sel")
            nome_vlan = st.text_input("Nome:", value=f"VLAN_{tenant['slug'].upper()}_{vlan_sel}_INT",
                                      key="vlan_int_nome")
...
with st.expander("4?? VLAN Transit", expanded=(
    ...
    if not st.session_state.tenant_criado:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            inicio = st.number_input("In\Uffffffffo:", 1, 4094, 800, key="vlan_tran_inicio")
        with col2:
        ...
        if st.session_state.get('vlans_tran_disp'):
            ...
            nome_vlan = st.text_input("Nome:", value=f"VLAN_{tenant['slug'].upper()}_{vlan_sel}_TRAN",
                                      key="vlan_tran_nome")
...
with st.expander("5?? Prefixo /24", expanded=(
        st.session_state.vlan_transit_alocada is not None and st.session_state.prefixo_24_alocado is None)):
    if not st.session_state.tenant_criado:
        st.warning("?? Crie um tenant primeiro")
        ...
        if st.button("?? Buscar /24", key="btn_buscar_24"):
            with st.spinner("Buscando prefixos /24 dispon\Uffffffffis..."):
        ...
        if st.session_state.get('prefixos_24'):
            ...
            if disp:
                st.success(f"? {len(disp)} prefixos /24 dispon\Uffffffffis")
                ...
                st.warning("?? Nenhum prefixo /24 dispon\Uffffffffl")

# ============================================================================
# ETAPA 6: PREFIXO /30
# ============================================================================

with st.expander("6?? Prefixo /30", expanded=(
        st.session_state.prefixo_24_alocado is not None and st.session_state.prefixo_30_alocado is None)):
    if not st.session_state.tenant_criado:
        st.warning("?? Crie um tenant primeiro")
        ...
        if st.button("?? Buscar /30", key="btn_buscar_30"):
            with st.spinner("Buscando prefixos /30 dispon\Uffffffffis..."):
        ...
        if st.session_state.get('prefixos_30'):
            ...
            if disp:
                st.success(f"? {len(disp)} prefixos /30 dispon\Uffffffffis")
                for i, p in enumerate(disp[:20], 1):
                    st.write(f"{i}. {p}")
                if len(disp) > 20:
                    st.caption(f"... e mais {len(disp) - 20}")
                ...
                st.warning("?? Nenhum prefixo /30 dispon\Uffffffffl")

# ============================================================================
# ETAPA 7: AWX \Uffffffff PROVISIONAR
# ============================================================================
...
if not all([tenant, vlan_int, vlan_tran, pref_24, pref_30]):
    st.warning("?? Complete todas as etapas anteriores")
else:
    ...
    st.subheader("?? Vari\Uffffffffis AWX")
            ...
            if erro:
                ...
                for i in range(30):
                    ...
                    if status == "successful":
                        status_container.success("? Conclu\Uffffffff!")
                        st.balloons()
                        break
                        ...
                        status_container.warning("?? Cancelado")
                        break
                    else:
                        status_container.info(f"?? {status.upper() if status else 'AGUARDANDO'}")
                        time.sleep(2)
    else:
        st.warning("?? Nenhum Job Template dispon\Uffffffffl")
...
st.markdown("---")
st.markdown(
    "<p style='text-align: center; color: #64748b;'>Portal NetBox + AWX | Python + Streamlit</p>",
    unsafe_allow_html=True
)
