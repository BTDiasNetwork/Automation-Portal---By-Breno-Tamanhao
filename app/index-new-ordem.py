...
with st.sidebar:
    ...
    <div style='text-align: center; margin-top: 8px;'>
        <span style='background: rgba(34,197,94,0.15); color: #4ade80; border-radius: 99px;
...
st.markdown(
    f"<p style='text-align: center; font-size: 1.05rem; color: #475569; margin-bottom: 0;'>"
...
with col1:
    ...
    <div style='background:white; border:1.5px solid #e2e8f0; border-left:4px solid #2563eb;
        ...
        <div style='flex:1; min-width:0;'>
            <div style='font-size:0.7rem; text-transform:uppercase; letter-spacing:1.2px;
                        color:#2563eb; font-weight:700; margin-bottom:2px;'>NetBox</div>
            <div style='font-size:1rem; font-weight:700; color:#0f172a;'>NetBox Community</div>
            <div style='font-size:0.78rem; color:#64748b; margin-top:1px;'>Versao {nb.version} &nbsp;·&nbsp; IPAM / DCIM</div>
        </div>
...
with col2:
    try:
        ...
        if templates_check:
            st.markdown(f"""
            <div style='background:white; border:1.5px solid #e2e8f0; border-left:4px solid #7c3aed;
                        border-radius:12px; padding:18px 22px; display:flex; align-items:center;
                        gap:16px; box-shadow:0 1px 6px rgba(0,0,0,0.06);'>
                ...
                <div style='flex:1; min-width:0;'>
                    <div style='font-size:0.7rem; text-transform:uppercase; letter-spacing:1.2px;
                                color:#7c3aed; font-weight:700; margin-bottom:2px;'>AWX / Ansible</div>
...
# ============================================================================
# SESSION STATE
# Nova ordem: tenant → vlan_internal → vlan_transit → vrf → prefixo_24 → prefixo_30
# ============================================================================
...
ETAPAS_LABELS = ["Tenant", "VLAN Int.", "VLAN Tran.", "VRF", "Prefixo /24", "Prefixo /30"]
...
for i, label in enumerate(ETAPAS_LABELS):
    ...
    cor_circulo = "background:linear-gradient(135deg,#2563eb,#0ea5e9);" if concluido else "background:#e2e8f0;"
...
progress_html = (
    "<div style='font-family:Inter,sans-serif;background:white;border:1.5px solid #e2e8f0;"
    ...
    "<div style='background:linear-gradient(90deg,#2563eb,#0ea5e9);height:100%;border-radius:99px;"
...
with st.expander("2️⃣  VLAN Internal", expanded=(
        st.session_state.tenant_criado is not None and st.session_state.vlan_internal_alocada is None)):
    if not st.session_state.tenant_criado:
        ...
        with col1:
            inicio = st.number_input("Inicio:", 1, 4094, 100, key="vlan_int_inicio")
        with col2:
...
with st.expander("3️⃣  VLAN Transit", expanded=(
    ...
    if not st.session_state.tenant_criado:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            inicio = st.number_input("Inicio:", 1, 4094, 800, key="vlan_tran_inicio")
        with col2:
...
with st.expander("4️⃣  Criar VRF", expanded=(
        st.session_state.vlan_transit_alocada is not None and st.session_state.vrf_criada is None)):
    if not st.session_state.tenant_criado:
        st.warning("⚠️ Crie um tenant primeiro")
        ...
        st.success(f"✅ VRF: **{st.session_state.vrf_criada['nome']}** — RD: `{st.session_state.vrf_criada['rd']}`")
    else:
        tenant   = st.session_state.tenant_criado
...
with st.expander("5️⃣  Prefixo /24", expanded=(
        st.session_state.vrf_criada is not None and st.session_state.prefixo_24_alocado is None)):
    if not st.session_state.tenant_criado:
        st.warning("⚠️ Crie um tenant primeiro")
...
<div style='background: white; border: 1.5px solid #e2e8f0; border-radius: 14px;
    ...
    <div style='display:flex; align-items:center; gap:12px;'>
        ...
        <div>
            <div style='font-size: 1.15rem; font-weight: 800; color: #1e3a5f;'>Provisionar AWX</div>
